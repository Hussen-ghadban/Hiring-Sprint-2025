...
export async function detectDamages(
  imageBuffer: Buffer,
  ...
  try {
    ...
    const jsonResponse = await axios({
      ...
      params: {
        ...
        format: "json",
        confidence: confidenceThreshold, // pass threshold to Roboflow API
      },
      data: base64Image,
...
function getIoU(a: Damage, b: Damage) {
...
// Compare damages using IoU
export function compareDamages(pickupDamages: Damage[], returnDamages: Damage[]) {
  const iouThreshold = 0.3; // Adjust: consider as same if IoU > 0.3
  ...
  for (const r of returnDamages) {
    ...
    for (const p of pickupDamages) {
      if (r.class === p.class && getIoU(r, p) > iouThreshold) {
        isNew = false;
        break;
      }
    }
